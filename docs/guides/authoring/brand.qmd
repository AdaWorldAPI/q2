...
It would be great if this were a demo in the same way that we have demos in /presentations/revealjs/index.qmd
...
This is similar to how [Themes](/guides/formats/html/themes.qmd#dark-mode) work.
...
::: {.list-table header-rows="1" widths="0.15789473684211,0.84210526315789"}
...
::: {.list-table header-rows="1" widths="0.25925925925926,0.43518518518519,0.2962962962963"}

* * Format
  * Location
  * Logo Preference (high to low)
...
When using `format: typst` you can additionally [customize the logo position](/guides/formats/typst/index.qmd#logo-position).

#### Disabling the logo

If you want to use `_brand.yml` for colors and typography but don't want the brand logo to appear on specific navigation elements, you can disable it in your `_quarto.yml`.
...
This is useful when you want brand styling applied to your project but prefer not to display a logo in the navigation.
...
::: {.list-table header-rows="1" widths="0.2,0.6,0.2"}
...
- For the `brand logo` shortcode, the default is `both`, which will expand to both light and dark logos with the appropriate [classes for mode-specific content](/guides/formats/html/themes.qmd#mode-specific-content) so that the light logo shows in light mode and the dark logo shows in dark mode.
...
## Sharing brands across projects {#sharing-brands-across-projects}

Organizations often maintain a shared brand that multiple projects should use. Quarto provides two ways to bring an external brand into your project: the `quarto use brand` command and Quarto Brand Extensions.

::: callout-warning
Quarto requires all brand assets---including logos and font files---to live within your project directory. Both methods described below copy brand files into your project rather than referencing them externally.

In most cases, these assets should be committed to source control.
:::
...
2.  If your project doesn't have a `_brand/` directory, Quarto asks to create one.
3.  For each file that already exists in your `_brand/` directory, Quarto asks whether to overwrite it.
4.  Quarto copies all files from the source to your `_brand/` directory.
5.  If there are files in your `_brand/` directory that don't exist in the source, Quarto asks whether to remove them.
...
Before making any changes, you can see exactly what would happen using the `--dry-run` option:

``` {.bash filename="Terminal"}
quarto use brand myorg/shared-brand --dry-run
```

This displays a summary of changes without modifying any files:
...
For scripts or automated workflows, use the `--force` option to skip all confirmation prompts:

``` {.bash filename="Terminal"}
quarto use brand myorg/shared-brand --force
```

With `--force`, Quarto trusts the source, overwrites conflicting files, creates new files, and removes files in your `_brand/` directory that aren't present in the source, all without prompting.

### Quarto Brand Extensions

Quarto also supports [brand extensions](../extensions/brand.qmd), a Quarto-specific way to distribute brand metadata and assets using a Git repository.
