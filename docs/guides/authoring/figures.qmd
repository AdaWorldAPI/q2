...
A figure is created whenever a captioned image appears by-itself in a paragraph. For example:
...
By default, figure captions are left aligned to accommodate longer caption text.

### Alt Text

You can add alternative text to a figure by adding the `fig-alt` attribute to the image. For example, the following Markdown generates the corresponding HTML with an `alt` tag:
...
Note that the figure caption, title, and alt text can all be different. For example:
...
To directly render the caption as alt text, append a backslash to the line^[This causes an empty additional character
to be added to the paragraph, which means the image is no longer recognizes as a figure.].
