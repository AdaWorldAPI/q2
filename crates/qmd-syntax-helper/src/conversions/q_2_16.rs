...
pub struct Q216Converter {}

#[derive(Debug, Clone)]
struct Q216Violation {
...
impl Q216Converter {
    ...
    /// Get parse errors and extract Q-2-16 unclosed superscript violations
    fn get_violations(&self, file_path: &Path) -> Result<Vec<Q216Violation>> {
        ...
        for diagnostic in diagnostics {
            // Check if this is a Q-2-16 error
            if diagnostic.code.as_deref() != Some("Q-2-16") {
                continue;
            }
    ...
    /// Apply fixes to the content by adding closing superscript marks
    fn apply_fixes(&self, content: &str, mut violations: Vec<Q216Violation>) -> Result<String> {
...
impl Rule for Q216Converter {
    fn name(&self) -> &str {
        "q-2-16"
    }

    fn description(&self) -> &str {
        "Fix Q-2-16: Add closing superscript marks for unclosed superscripts"
    }
    ...
    fn check(&self, file_path: &Path, _verbose: bool) -> Result<Vec<CheckResult>> {
        ...
        let results: Vec<CheckResult> = violations
            ...
            .map(|v| CheckResult {
                ...
                issue_count: 1,
                message: Some(format!(
                    "Q-2-16 unclosed superscript at offset {}",
                    v.offset
                )),
    ...
    fn convert(
        ...
        if violations.is_empty() {
            return Ok(ConvertResult {
                ...
                file_path: file_path.to_string_lossy().to_string(),
                fixes_applied: 0,
                message: Some("No Q-2-16 unclosed superscript issues found".to_string()),
            });
        }
        ...
        if check_mode {
            ...
            return Ok(ConvertResult {
                ...
                message: Some(format!(
                    "Would fix {} Q-2-16 unclosed superscript violation(s)",
                    violations.len()
                )),
            });
        }
        ...
        if in_place {
            ...
            Ok(ConvertResult {
                ...
                fixes_applied: violations.len(),
                message: Some(format!(
                    "Fixed {} Q-2-16 unclosed superscript violation(s)",
                    violations.len()
                )),
            })
        } else {
