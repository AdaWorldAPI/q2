...
pub struct Q222Converter {}

#[derive(Debug, Clone)]
struct Q222Violation {
...
impl Q222Converter {
    ...
    /// Get parse errors and extract Q-2-22 unclosed editorial highlight violations
    fn get_violations(&self, file_path: &Path) -> Result<Vec<Q222Violation>> {
        ...
        for diagnostic in diagnostics {
            // Check if this is a Q-2-22 error
            if diagnostic.code.as_deref() != Some("Q-2-22") {
                continue;
            }
    ...
    fn check(&self, file_path: &Path, _verbose: bool) -> Result<Vec<CheckResult>> {
        ...
        let results: Vec<CheckResult> = violations
            ...
            .map(|v| CheckResult {
                ...
                issue_count: 1,
                message: Some(format!(
                    "Q-2-22 unclosed editorial highlight at offset {}",
                    v.offset
                )),
    ...
    fn convert(
        ...
        if violations.is_empty() {
            return Ok(ConvertResult {
                ...
                file_path: file_path.to_string_lossy().to_string(),
                fixes_applied: 0,
                message: Some("No Q-2-22 unclosed editorial highlight issues found".to_string()),
            });
        }
        ...
        if check_mode {
            ...
            return Ok(ConvertResult {
                ...
                message: Some(format!(
                    "Would fix {} Q-2-22 unclosed editorial highlight violation(s)",
                    violations.len()
                )),
            });
        }
        ...
        if in_place {
            ...
            Ok(ConvertResult {
                ...
                fixes_applied: violations.len(),
                message: Some(format!(
                    "Fixed {} Q-2-22 unclosed editorial highlight violation(s)",
                    violations.len()
                )),
            })
        } else {
