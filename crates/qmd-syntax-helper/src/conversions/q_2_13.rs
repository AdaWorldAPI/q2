...
//
// Example:
//   Input:  **This is an unclosed strong
//   Output: **This is an unclosed strong**
//
...
pub struct Q213Converter {}

#[derive(Debug, Clone)]
struct Q213Violation {
...
impl Q213Converter {
    ...
    /// Get parse errors and extract Q-2-13 unclosed strong star emphasis violations
    fn get_violations(&self, file_path: &Path) -> Result<Vec<Q213Violation>> {
        ...
        for diagnostic in diagnostics {
            // Check if this is a Q-2-13 error
            if diagnostic.code.as_deref() != Some("Q-2-13") {
                continue;
            }
    ...
    fn apply_fixes(&self, content: &str, mut violations: Vec<Q213Violation>) -> Result<String> {
...
impl Rule for Q213Converter {
    fn name(&self) -> &str {
        "q-2-13"
    }

    fn description(&self) -> &str {
        "Fix Q-2-13: Add closing '**' marks for unclosed strong emphasis"
    }
    ...
    fn check(&self, file_path: &Path, _verbose: bool) -> Result<Vec<CheckResult>> {
        ...
        let results: Vec<CheckResult> = violations
            ...
            .map(|v| CheckResult {
                ...
                issue_count: 1,
                message: Some(format!(
                    "Q-2-13 unclosed strong star emphasis at offset {}",
                    v.offset
                )),
    ...
    fn convert(
        ...
        if violations.is_empty() {
            return Ok(ConvertResult {
                ...
                file_path: file_path.to_string_lossy().to_string(),
                fixes_applied: 0,
                message: Some("No Q-2-13 unclosed strong star emphasis issues found".to_string()),
            });
        }
        ...
        if check_mode {
            ...
            return Ok(ConvertResult {
                ...
                message: Some(format!(
                    "Would fix {} Q-2-13 unclosed strong star emphasis violation(s)",
                    violations.len()
                )),
            });
        }
        ...
        if in_place {
            ...
            Ok(ConvertResult {
                ...
                fixes_applied: violations.len(),
                message: Some(format!(
                    "Fixed {} Q-2-13 unclosed strong star emphasis violation(s)",
                    violations.len()
                )),
            })
        } else {
