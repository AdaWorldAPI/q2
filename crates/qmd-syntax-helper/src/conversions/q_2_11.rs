...
//
// Example:
//   Input:  "This is an unclosed quote
//   Output: "This is an unclosed quote"
//
...
pub struct Q211Converter {}

#[derive(Debug, Clone)]
struct Q211Violation {
    offset: usize,                          // Offset where closing quote should be added
...
impl Q211Converter {
    ...
    /// Get parse errors and extract Q-2-11 unclosed double quote violations
    fn get_violations(&self, file_path: &Path) -> Result<Vec<Q211Violation>> {
        ...
        for diagnostic in diagnostics {
            // Check if this is a Q-2-11 error
            if diagnostic.code.as_deref() != Some("Q-2-11") {
                continue;
            }
    ...
    /// Apply fixes to the content by adding closing double quotes
    fn apply_fixes(&self, content: &str, mut violations: Vec<Q211Violation>) -> Result<String> {
        ...
        for violation in violations {
            // Insert closing double quote at the error location (end of block)
...
impl Rule for Q211Converter {
    fn name(&self) -> &str {
        "q-2-11"
    }

    fn description(&self) -> &str {
        "Fix Q-2-11: Add closing double quotes for unclosed quote marks"
    }
    ...
    fn check(&self, file_path: &Path, _verbose: bool) -> Result<Vec<CheckResult>> {
        ...
        let results: Vec<CheckResult> = violations
            ...
            .map(|v| CheckResult {
                ...
                issue_count: 1,
                message: Some(format!(
                    "Q-2-11 unclosed double quote at offset {}",
                    v.offset
                )),
    ...
    fn convert(
        ...
        if violations.is_empty() {
            return Ok(ConvertResult {
                ...
                file_path: file_path.to_string_lossy().to_string(),
                fixes_applied: 0,
                message: Some("No Q-2-11 unclosed double quote issues found".to_string()),
            });
        }
        ...
        if check_mode {
            ...
            return Ok(ConvertResult {
                ...
                message: Some(format!(
                    "Would fix {} Q-2-11 unclosed double quote violation(s)",
                    violations.len()
                )),
            });
        }
        ...
        if in_place {
            ...
            Ok(ConvertResult {
                ...
                fixes_applied: violations.len(),
                message: Some(format!(
                    "Fixed {} Q-2-11 unclosed double quote violation(s)",
                    violations.len()
                )),
            })
        } else {
