// Q-2-21: Unclosed Editorial Comment
//
// This conversion rule fixes Q-2-21 errors by adding closing ']' marks
...
// Message: "I reached the end of the block before finding a closing ']' for the editorial comment."
//
// Example:
//   Input:  [>>This is an unclosed comment
//   Output: [>>This is an unclosed comment]
//
...
pub struct Q221Converter {}

#[derive(Debug, Clone)]
struct Q221Violation {
...
impl Q221Converter {
    ...
    /// Get parse errors and extract Q-2-21 unclosed editorial comment violations
    fn get_violations(&self, file_path: &Path) -> Result<Vec<Q221Violation>> {
        ...
        for diagnostic in diagnostics {
            // Check if this is a Q-2-21 error
            if diagnostic.code.as_deref() != Some("Q-2-21") {
                continue;
            }
    ...
    fn apply_fixes(&self, content: &str, mut violations: Vec<Q221Violation>) -> Result<String> {
...
impl Rule for Q221Converter {
    fn name(&self) -> &str {
        "q-2-21"
    }

    fn description(&self) -> &str {
        "Fix Q-2-21: Add closing ']' for unclosed editorial comment"
    }
    ...
    fn check(&self, file_path: &Path, _verbose: bool) -> Result<Vec<CheckResult>> {
        ...
        let results: Vec<CheckResult> = violations
            ...
            .map(|v| CheckResult {
                ...
                issue_count: 1,
                message: Some(format!(
                    "Q-2-21 unclosed editorial comment at offset {}",
                    v.offset
                )),
    ...
    fn convert(
        ...
        if violations.is_empty() {
            return Ok(ConvertResult {
                ...
                file_path: file_path.to_string_lossy().to_string(),
                fixes_applied: 0,
                message: Some("No Q-2-21 unclosed editorial comment issues found".to_string()),
            });
        }
        ...
        if check_mode {
            ...
            return Ok(ConvertResult {
                ...
                message: Some(format!(
                    "Would fix {} Q-2-21 unclosed editorial comment violation(s)",
                    violations.len()
                )),
            });
        }
        ...
        if in_place {
            ...
            Ok(ConvertResult {
                ...
                fixes_applied: violations.len(),
                message: Some(format!(
                    "Fixed {} Q-2-21 unclosed editorial comment violation(s)",
                    violations.len()
                )),
            })
        } else {
