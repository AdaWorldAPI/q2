...
//
// This conversion rule fixes Q-2-20 errors by adding closing '--]' marks
...
// Message: "I reached the end of the block before finding a closing '--]' for the editorial delete."
//
// Example:
//   Input:  [--This is an unclosed delete
//   Output: [--This is an unclosed delete--]
//
...
pub struct Q220Converter {}

#[derive(Debug, Clone)]
struct Q220Violation {
...
impl Q220Converter {
    ...
    /// Get parse errors and extract Q-2-20 unclosed editorial delete violations
    fn get_violations(&self, file_path: &Path) -> Result<Vec<Q220Violation>> {
        ...
        for diagnostic in diagnostics {
            // Check if this is a Q-2-20 error
            if diagnostic.code.as_deref() != Some("Q-2-20") {
                continue;
            }
    ...
    fn apply_fixes(&self, content: &str, mut violations: Vec<Q220Violation>) -> Result<String> {
...
impl Rule for Q220Converter {
    fn name(&self) -> &str {
        "q-2-20"
    }

    fn description(&self) -> &str {
        "Fix Q-2-20: Add closing '--]' marks for unclosed editorial delete"
    }
    ...
    fn check(&self, file_path: &Path, _verbose: bool) -> Result<Vec<CheckResult>> {
        ...
        let results: Vec<CheckResult> = violations
            ...
            .map(|v| CheckResult {
                ...
                issue_count: 1,
                message: Some(format!(
                    "Q-2-20 unclosed editorial delete at offset {}",
                    v.offset
                )),
    ...
    fn convert(
        ...
        if violations.is_empty() {
            return Ok(ConvertResult {
                ...
                file_path: file_path.to_string_lossy().to_string(),
                fixes_applied: 0,
                message: Some("No Q-2-20 unclosed editorial delete issues found".to_string()),
            });
        }
        ...
        if check_mode {
            ...
            return Ok(ConvertResult {
                ...
                message: Some(format!(
                    "Would fix {} Q-2-20 unclosed editorial delete violation(s)",
                    violations.len()
                )),
            });
        }
        ...
        if in_place {
            ...
            Ok(ConvertResult {
                ...
                fixes_applied: violations.len(),
                message: Some(format!(
                    "Fixed {} Q-2-20 unclosed editorial delete violation(s)",
                    violations.len()
                )),
            })
        } else {
