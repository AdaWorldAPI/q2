...
- **Phase 1 — annotate stage (native)**:
  ...
  - [ ] Native user-grammar loader via `tree_sitter::WasmStore` (wasmtime always included on native); scans `_quarto/grammars/<lang>/` at project-load
  - [ ] Pipeline stage: walks AST, highlights `CodeBlock` **and inline `Code`** with a resolvable class, writes `data-hl-spans` as a JSON triple-array
  - [ ] Skip if `data-hl-spans` already present (filter-produced annotations win)
