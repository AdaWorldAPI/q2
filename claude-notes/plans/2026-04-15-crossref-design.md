...
- [ ] 2.2 Callout integration: callouts already sugarify; extend to index callouts with explicit ids (`#nte-foo`). **Deferred** — details below.

  **Why deferred.** The shared crossref infrastructure (`crossref_target_view`, `CrossrefIndexTransform`, `CrossrefResolveTransform`) already works for *any* `CustomNode` that carries the standard `plain_data` triple (`ref_type`, `kind`, `identifier`). Theorems proved this: no indexer or resolver code had to change. So the only missing piece for callouts is populating those three fields on the `Callout` `CustomNode` when the author gives it a crossref-eligible id.

  **What needs to happen.**
  1. In `CalloutTransform` (`crates/quarto-core/src/transforms/callout.rs`), after building the `CustomNode("Callout")`, check whether `attr.0` (the identifier) classifies as a crossref via `RefTypeRegistry::classify_cite_id`. If it does, inject `ref_type`, `kind`, and `identifier` into `plain_data` alongside the existing callout-specific fields (`type`, `appearance`, etc.). The transform currently doesn't receive the registry — it would need `RenderContext.ref_type_registry` threaded through its `transform(&self, ast, ctx)` call (it already receives `ctx` but ignores it today).
  2. Map the callout *type* (note, warning, tip, caution, important) to the matching ref-type prefix already in the built-in registry: `nte`, `wrn`, `tip`, `cau`, `imp`. The mapping is static; a small lookup table suffices.
  3. `CrossrefRenderTransform` already handles `FloatRefTarget` and `Theorem`. Callouts are rendered by `CalloutResolveTransform` *before* the crossref render phase, so by the time the render transform runs, the callout is already a structured `Div` — no additional render code needed for the crossref side. The resolver would produce `<a href="#nte-foo">Note 1</a>` using the standard path.
  4. Pipeline ordering: `CalloutTransform` runs early in the normalization phase, before `FloatRefTargetSugarTransform` and `CrossrefIndexTransform`. This is already the correct position — the callout becomes a `CustomNode("Callout")` with crossref fields, and the indexer picks it up later.
  5. No implicit auto-labeling (Q1 behavior): only callouts with an author-supplied `#<prefix>-<id>` get indexed. Callouts without an id stay unnumbered.

  **What does NOT need to happen.** No changes to `crossref_target_view`, `CrossrefIndexTransform`, `CrossrefResolveTransform`, or `CrossrefRenderTransform` — the plain_data triple is the only integration point, and it's already the canonical contract.

  **Why not do it now.** `CalloutTransform` currently takes `_ctx: &mut RenderContext` and ignores it. Threading the registry through requires the callout transform to read `ctx.ref_type_registry`, which is a small but cross-cutting change touching a well-tested transform. Doing it cleanly means also updating the callout transform's tests (which construct `RenderContext` without a registry today). The payoff is real but narrow — callout crossrefs are uncommon compared to figures and theorems — so we deferred it rather than risk destabilizing the callout path while the theorem/proof work was in flight.
